...
float ssao()
{
    ...
    vec3 normal = normalize((texture(u_normalRoughness, v_texCoord).xyz * 2.0) - vec3(1.0));
    ivec2 texDim = textureSize(u_depth, 0);
    ivec2 noiseDim = textureSize(u_ssaoNoise, 0);
    vec2 noiseUV = vec2(float(texDim.x) / float(noiseDim.x), float(texDim.y) / float(noiseDim.y)) * v_texCoord;
